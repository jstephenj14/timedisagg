...
from src.assemble import CalcGLS

def fill_off_diag(matrix, val):
...
def calculate_Q( pm, rho):
    return (1 / (1 - rho ** 2)) * (rho ** pm)

def calculate_dyn_adj(X, rho):
    n = len(X)
    diag = np.identity(n)
    diag_rho = fill_off_diag(diag, -rho)
...
def objective_fn(rho):
    X_adj = calculate_dyn_adj(X, rho)
    X_l_adj = C.dot(X_adj)
    print("In objective dyn: " + str(X_l_adj.shape))
    Q = calculate_Q(pm, rho)
    vcov = (C.dot(Q)).dot(C.T)
    return -1 * CalcGLS(y_l, X_l_adj, vcov)["logl"]
...
# data adjustment
X_dash = calculate_dyn_adj(X, rho_min)

X_l_adj = C.dot(X_dash)
...
##################################### Dissembling Calcgls #####################################################
# z = CalcGLS(y_l, X_l_adj, Q_l_real, stats=True)
...
u_l = y_l.reshape(len(y_l), 1) - C.dot(p).reshape(len(C.dot(p)),1)
